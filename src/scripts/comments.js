...
(function() {
  ...
  const renderComments = commentsList =>
    commentsList.forEach(item => {
      ...
      <section class="user-comment__data">
        ...
        <section class="user-comment__name">${item.name} (${
        item.email
      })</section>
      ...
      <section class="wrapper-comment-interactions">
        <button class="reply-comment-btn" type="button" onclick="replyThisComment('${
          item.name
        }', ${item.id})">REPLY</button>
        <button class="comment-replies-btn" id="replies-${
          item.id
        }" onclick="renderSubComments(${item.id})">
          Show replied comments (${
            subComments[item.id] !== undefined ? subComments[item.id].length : 0
          })
        </button>
      </section>
  ...
  // Button which activates comment field
  ...
  function activateComments() {
    ...
    document
      .querySelector(".comments-field")
      .classList.toggle("comments-field--active");
    commentBundleIndex = 0;
    renderCommentBundleBtns();
    document
      .querySelector(`#comments-bottom0`)
      .classList.add("comments-nav__btn--active");
    document
      .querySelector(`#comments-top0`)
      .classList.add("comments-nav__btn--active");
    return renderComments(
      comments.slice(comments.length >= 15 ? comments.length - 15 : 0)
    );
  }

  showCommentsBtn.addEventListener("click", activateComments);

  // Button which adds a new comment and renders it immediately

  const addCommentButton = () => {
    if (
      commentatorName.value === "" ||
      commentatorEmail.value === "" ||
      commentatorText.value === ""
    ) {
      return;
    }
  ...
  function renderCommentBundleBtns() {
    ...
    createBtn(
      ...
      "<",
      topCommentsNav,
      `prevCommentBundle(${commentBundleIndex - 1})`
    );
    createBtn(
      "comments-nav__btn",
      "comments-nav__prev",
    ...
    createBtn(
      ...
      ">",
      topCommentsNav,
      `nextCommentBundle(${commentBundleIndex + 1})`
    );
    createBtn(
      "comments-nav__btn",
      "comments-nav__next",
  ...
  function renderCertainComments(index) {
    ...
    if (commentBundleIndex !== undefined) {
      document
        .querySelector(`#comments-bottom${commentBundleIndex}`)
        .classList.remove("comments-nav__btn--active");
      document
        .querySelector(`#comments-top${commentBundleIndex}`)
        .classList.remove("comments-nav__btn--active");
    }
    ...
    renderCommentBundleBtns();
    document
      .querySelector(`#comments-bottom${index}`)
      .classList.add("comments-nav__btn--active");
    document
      .querySelector(`#comments-top${index}`)
      .classList.add("comments-nav__btn--active");
  ...
  function replyThisComment(commentatorName, commentId) {
    ...
    showCommentsBtn.addEventListener("click", activateComments);
    addSubCommentBtn.setAttribute(
      "onclick",
      `addSubCommentButton('${commentatorName}', ${commentId})`
    );
  }
  ...
  function renderSubComments(parrentCommentId) {
    const parentCommentReplies = document.getElementById(
      `comment-replies-${parrentCommentId}`
    );
    parentCommentReplies.innerHTML = "";
    const parentCommentRepliesBtn = document.getElementById(
      `replies-${parrentCommentId}`
    );
    ...
    subComments[parrentCommentId].forEach(item => {
      let out = `
        ...
        item.email
          ...
          <section class="user-comment__date">${convertTime(
            item.time
          )}</section>
        </section>
        <p class="user-comment__text">
          <a class="commentator-anchor" href="#anchor-${
            item.parrentCommentId
          }">${item.parrentCommentName},</a>
          ${item.body}
        </p>
        <section class="wrapper-comment-interactions">
          <button class="reply-comment-btn" type="button" onclick="replyThisComment('${
          ...
          <button class="comment-replies-btn" id="replies-${
            ...
            Show replied comments (${
              subComments[item.id] !== undefined
                ? subComments[item.id].length
                : 0
            })
          </button>
        </section>
    ...
    `;
      ...
      comment.setAttribute("id", `comment-${item.id}`);
      comment.innerHTML = out;
      parentCommentReplies.insertBefore(
        comment,
        parentCommentReplies.firstChild
      );
    });
